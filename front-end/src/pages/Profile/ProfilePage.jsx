...
const ProfilePage = () => {
    ...
    const mutation = useMutationHooks(
        async (data) => {
            ...
            return { id, access_token }; // Trả về để dùng sau
        }
    );
    ...
    useEffect(() => {
        if (isSuccess && mutation.data) {
            setLoading(true)

            const { id, access_token } = mutation.data;
            message.success("Cập nhật thành công");
    ...
    return (
        <div style={{ width: '100%', margin: '20px auto', display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
